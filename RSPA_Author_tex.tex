...
\begin{split}
&\frac{du}{d\tau}=(\vec{\alpha}_1+\textrm{i}\vec{\beta}_1)\nu u+(\vec{\alpha}_2+\textrm{i}\vec{\beta}_2)\nu \bar{u}+\sum_k g_k(u,\bar{u}),\\
...
\begin{split}
\frac{dR}{d\tau}=&\vec{a}_{1101}\nu u+(a_{1210}-a_{1200}a_{2110}-a_{2200}a_{1110})R^3\\
...
\noindent \Eref{eq:13} has codimension-3 Hopf bifurcation at $\lambda=0$. As explained in \Ssref{SNF}, we can derive the reduced dynamics on center manifold of \Eref{eq:13} and compute SNF using nonlinear transformation between origianl coordinates $[x,\bar x,\lambda,t]^T$ and normal form coordinates $[u,\bar u,\nu,\tau]^T$. We will study the stabilizability of unstable limit cycle of the uncontroled system at $\lambda=\lambda_1=[\mu,0,0]^T$ by using control based continuation. Limit cycle($[u_1(\tau),\bar u_1(\tau)]^T$) is a nonzero fixed point of \Eref{eq:10} and expressed in normal form coordinates $[u,\bar u]^T$ as
...
\noindent Note that $\omega_1$ in \Eref{eq:16} is rescaled frequency of LCO from $\omega_0$ at $\lambda=\lambda_0$. By putting \Eref{eq:17} to \Eref{eq:11} nonautonomous term of \Eref{eq:11} is a single harmonic oscillator, and our problem to be solved is Hopf bifurcation with single harmonic forcing. The normal form of forced Hopf bifurcation for small amplitude bounded solution is equivalent to adding force in the direction of the centerspace to the autonomous Hopf normal form \cite{golubitsky2012feed,elphick1987normal}. It can be shown\cite{guckenheimer1983nonlinear} that for $t>\textrm{inf}(\gamma)$, where $\gamma$ is eigenvalue of $J_s$, dynamics on the stable manifold does not affect the final response. Therefore, if we define $\epsilon$ as an amplitude of periodic forcing which is rescaled in the normal form coordinates, normal form with single harmonic forcing at $\lambda=\lambda_2$ can be derived as

\begin{align}\label{eq:18}
  \frac{du}{d\tau}=(\alpha_1+\textrm{i}\beta_1)u+(\alpha_2+\textrm{i}\beta_2)u^2+\epsilon e^{\textrm{i}\omega_2\tau},
\end{align}
...
\frac{du}{d\tau}=(\alpha_1+\textrm{i}\beta_1)u+(\alpha_2+\textrm{i}\beta_2)u^2+\epsilon e^{\textrm{i}\theta}.
\end{align}
...
We use a real-time controller to achieve phase locking between target coefficients $z^*(t)$ with the response in \Eref{eq:11}. This is equivalent to phase-locking between force and response in \Eref{eq:20}, which was first proposed in \cite{irene}. We assume that monitored signals $z_j$, $z_{j+1}$ can be expressed as the analytic signal,
...
\noindent where, $\vec{C}=diag(1,0,1,0)$ is matrix that relates state variable and measured output, $\vec{w}$ is measurement noise, and $\vec{A}_{T_s}$ can be computed as
